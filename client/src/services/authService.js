...
const checkUserByEmail = async (email) => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || error.message || "Email check failed"
    );
  }
};
...
const pendingRegLogin = async (email) => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || error.message || "Pending login failed"
    );
  }
};
...
const disable2FA = async (data) => {
  try {
    const response = await axios.post(API_URL + "disable-2fa", data, {
      withCredentials: true,
    });
    return response.data;
  } catch (error) {
