...
const RegisterStep6 = () => {
  ...
  useEffect(() => {
    if (user && referredBy && Settings) {
      ...
      if (user.requiredReferrals <= 0 && !hasMembershipFee()) {
        navigate("/homepage", { replace: true });
        return;
      }
  ...
  const handleProceedToSendReferredBy = async () => {
    if (referredBy.length >= user.requiredReferrals) {
      toast.error("You've reached the limit of required members", {
        duration: 3_000,
      });
      return;
    }
    ...
    if (selectedMembers.size > remainingReferred && referredBy.length) {
      const message = `You can refer up to ${remainingReferred} member${
        remainingReferred > 1 ? "s" : ""
      }.`;
    ...
    if (
      referredBy.length === 0 &&
      selectedMembers.size > user.requiredReferrals
    ) {
      const message = `Only ${user.requiredReferrals} member${
        user.requiredReferrals > 1 ? "s" : ""
      }. are required`;
      toast.error(message, { duration: 3_000 });
      return;
    }
    ...
    try {
      ...
      selectedMembers.forEach((id) =>
        payload.push(members.find((m) => m.id === id).id)
      );
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {referredByLoading ? (
        <div className="min-h-[45em] max-w-xl m-auto w-full flex justify-center items-center">
        ...
        <>
          {approval ? (
            <RegisterApproval
              ...
              onDecliningReferredBy={() => refetchMembersAndReferredBy()}
            />
          ) : (
            <div className="container mx-auto px-4 py-8 md:py-12 max-w-3xl flex-grow">
              ...
              <div className="bg-[#E3F8F959] rounded-lg p-4 md:p-6">
                ...
                <p className="text-gray-500 text-xs md:text-sm mt-4">
                  ...
                  {Settings?.requiredReferralNumber ?? 3} members
                </p>
              </div>
