...
const YourEvents = () => {
  ...
  const { affiliates = [], isLoading: affiliatesLoading } = useSelector((state) => state.affiliate || {});
  ...
  useEffect(() => {
    if (selectedEvent && isEditEventOpen) {
      ...
      setEventFormData({
        ...
        affiliatePartner: selectedEvent.affiliatePartner || "",
      })
  ...
  const handleCreateEvent = () => {
    ...
    formData.append("affiliatePartner", eventFormData.affiliatePartner)
    ...
    if (evidenceFiles.length > 0) {
      evidenceFiles.forEach((file) => {
        formData.append("evidenceOfPlan", file)
      })
    }
  ...
  const handleUpdateEvent = () => {
    ...
    // In handleUpdateEvent, exclude removed evidence URLs from the update payload
    if (selectedEvent.evidenceOfPlan && selectedEvent.evidenceOfPlan.length > 0) {
      const keptEvidence = selectedEvent.evidenceOfPlan.filter(url => !removedEvidenceUrls.includes(url))
      formData.append('existingEvidenceOfPlan', JSON.stringify(keptEvidence))
    }
        ...
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block mb-1">Affiliate Partner</label>
                <select
                  className="w-full px-4 py-2 border rounded-lg text-gray-600 bg-white"
                  name="affiliatePartner"
                  value={eventFormData.affiliatePartner}
                  onChange={handleInputChange}
              ...
              <div className="border rounded-lg h-[120px] overflow-y-auto">
                {usersLoading ? (
                  ...
                  filteredUsers.map((user) => (
                    <div
                      key={user._id}
                      className={`items-center gap-2 p-2 ${selectedUsers.some((u) => u._id === user._id) ? "hidden" : "flex"
                        }`}
                    >
                      <input
                        type="checkbox"
            ...
            <div className="w-full md:w-7/12 overflow-y-auto max-h-[80vh] will-change-transform">
              <div className="border-b">
                <div className="flex overflow-x-auto">
                  <button
                    className={`px-4 md:px-8 py-3 md:py-4 whitespace-nowrap ${activeTab === "description" ? "bg-[#540A26] text-white" : "bg-white text-black"
                      }`}
                  ...
                  <button
                    className={`px-4 md:px-8 py-3 md:py-4 whitespace-nowrap ${activeTab === "location" ? "bg-[#540A26] text-white" : "bg-white text-black"
                      }`}
                  ...
                  <button
                    className={`px-4 md:px-8 py-3 md:py-4 whitespace-nowrap ${activeTab === "members" ? "bg-[#540A26] text-white" : "bg-white text-black"
                      }`}
