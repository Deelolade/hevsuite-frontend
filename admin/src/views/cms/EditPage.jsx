...
const EditPage = ({ onBack, selectedItem }) => {
  ...
  useEffect(() => {
    if (selectedItem && !initialDataLoaded) {
      ...
      // Set footer ID if available
      if (selectedItem.footerId) {
        setFooterId(selectedItem.footerId)
      }
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      let currentFooterId = footerId

      if (!currentFooterId && selectedItem?._id) {
        // Find which footer contains this item
        for (const footer of footers) {
          if (footer.items && footer.items.some((item) => item._id === selectedItem._id)) {
            currentFooterId = footer._id
            break
          }
        }
      }
      ...
      // Find the current footer
      const currentFooter = footers.find((f) => f._id === currentFooterId)
      if (!currentFooter) {
      ...
      const updatedItems = currentFooter.items.map((item) => (item._id === selectedItem._id ? updatedPage : item))
